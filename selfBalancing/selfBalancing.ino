...
#include <TimerOne.h>        // Require Timer1 PWM frequency of 20Khz-25Khz for Nidec 24H677 BLDC Motor
...
const int Direction = 7; // BLDC Dir
const int PWM = 9;     // BLDC PWM
...
#define Halleffect 12  // External interrupt D12 for Nidec FG output 6 Pulses/Revoltion
...
void setup() 
{
  ...
  pinMode(PWM,OUTPUT);
  pinMode(Halleffect, INPUT);  //speed encoder input
  pinMode(13, OUTPUT);
  
  // Output initial state values
  digitalWrite(Direction,LOW);
  digitalWrite(Direction,HIGH); //Red LED ON // Reaction Wheel home resting position
  analogWrite(PWM,0); // No PWM Signal
...
void loop() 
{
  ...
  attachPinChangeInterrupt(Halleffect, Code_Hall, CHANGE);  
}
...
void ReactionWheelPWM()
{
 ...
 //Serial.println(pwmOut);
  ...
  if (angle >= 25) // Reaction Wheel Right Side Jump-up Position 
   {
    ...
    digitalWrite(Direction,LOW); // Nidec Direction CW
    analogWrite(PWM,185);
    delay(4000);
    digitalWrite(Direction,HIGH); // Nidec Direction CCW
    analogWrite(PWM,10);
    pwmOut2 = 0;
    delay(125);
   } 
  ...
  if(angle >= 20 || angle <= -20)  // if angle is greater than +/- 20° than the Nidec motor will stop
      {                                      
      digitalWrite(Brake,LOW);  // Nidec motor brake ON
      analogWrite(PWM, 0);
      }
      else
      {
